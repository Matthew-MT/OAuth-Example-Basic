...
import { active, refresh, requested } from "../utility/faux_database.js";
    ...
    switch (req.body.grant_type) {
        ...
        case "refresh_token": {
            if (
                ...
                // ^ Above: Require refresh_token in request body
            ) {
                res.status(401).send();
                return;
            }
            ...
            if (
                !refresh.has(req.body.refresh_token)
                // ^ Above: Validate refresh token
            ) {
                res.status(400).send();
                return;
            }
